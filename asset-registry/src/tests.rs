...
// 500); 		assert_eq!(ParaTokens::free_balance(CurrencyId::B, &BOB), 460);
//
// 		assert_ok!(ParaXTokens::transfer(
// 			Some(BOB).into(),
// 			CurrencyId::A,
...
// 500); 		assert_eq!(ParaTokens::free_balance(CurrencyId::A, &ALICE), 460);
// 	});
// }
//
// #[test]
...
// 	ParaB::execute_with(|| {
// 		assert_eq!(ParaTokens::free_balance(CurrencyId::B, &sibling_a_account()),
// 550); 		assert_eq!(ParaTokens::free_balance(CurrencyId::B1,
// &sibling_a_account()), 950); 		assert_eq!(ParaTokens::free_balance(CurrencyId::
// B, &BOB), 450); 		assert_eq!(ParaTokens::free_balance(CurrencyId::B1, &BOB),
// 10); 	});
// }
//
// #[test]
...
// 	ParaB::execute_with(|| {
// 		assert_eq!(ParaTokens::free_balance(CurrencyId::B, &sibling_a_account()),
// 500); 		assert_eq!(ParaTokens::free_balance(CurrencyId::B,
// &sibling_c_account()), 460); 	});
//
// 	ParaC::execute_with(|| {
...
fn send_self_parachain_asset_to_sibling() {
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(ParaTokens::deposit(CurrencyId::ForeignAsset(0), &ALICE, 1_000));
		...
		assert_eq!(ParaTokens::free_balance(CurrencyId::ForeignAsset(0), &ALICE), 500);
		assert_eq!(
			ParaTokens::free_balance(CurrencyId::ForeignAsset(0), &sibling_b_account()),
			500
		);
	});

	ParaB::execute_with(|| {
		assert_eq!(ParaTokens::free_balance(CurrencyId::ForeignAsset(0), &BOB), 460);
	});
}
//
// #[test]
...
//
// 		assert_eq!(ParaTokens::free_balance(CurrencyId::A, &sibling_b_account()),
// 450); 		assert_eq!(ParaTokens::free_balance(CurrencyId::A1,
// &sibling_b_account()), 50); 	});
//
...
// 		assert_eq!(1000 - fee_amount, ParaTokens::free_balance(CurrencyId::R,
// &ALICE)); 	});
//
// 	Relay::execute_with(|| {
// 		assert_eq!(
...
// 		assert_eq!(39, ParaTokens::free_balance(CurrencyId::R,
// &sibling_a_account()));
//
// 		// second xcm failed, so recipient account don't receive any token of B and
// R. 		assert_eq!(0, ParaTokens::free_balance(CurrencyId::B, &BOB));
// 		assert_eq!(0, ParaTokens::free_balance(CurrencyId::R, &BOB));
// 	});
// }
//
...
//
// 	Relay::execute_with(|| {
// 		let _ = RelayBalances::deposit_creating(&para_a_account(),
// 1_000_000_000_000); 	});
//
// 	ParaA::execute_with(|| {
...
//
// 		let call =
// 			relay::Call::System(frame_system::Call::<relay::Runtime>::remark_with_event {
// remark: vec![1, 1, 1] }); 		let assets: MultiAsset = (Here,
// 1_000_000_000_000).into(); 		assert_ok!(para::OrmlXcm::send_as_sovereign(
// 			para::Origin::root(),
// 			Box::new(Parent.into()),
...
// 			relay::Call::System(frame_system::Call::<relay::Runtime>::remark_with_event {
// remark: vec![1, 1, 1] }); 		let assets: MultiAsset = (Here,
// 1_000_000_000_000).into(); 		assert_err!(
...
// 		"size of Call is more than 200 bytes: some calls have too big arguments, use
...
// 			matches!(
// 				r.event,
// 				para::Event::PolkadotXcm(pallet_xcm::Event::<para::Runtime>::AssetsTrapped(_,
// _, _)) 			)
// 		}));
// 	})
// }
...
//
// 	ParaD::execute_with(|| {
// 		assert_ok!(ParaRelativeTokens::deposit(CurrencyId::D, &sibling_a_account(),
// 1_000)); 		assert_ok!(ParaRelativeTokens::deposit(CurrencyId::A, &BOB, 1_000));
// 	});
//
// 	ParaA::execute_with(|| {
// 		assert_ok!(ParaXTokens::transfer(
...
//
// 	ParaA::execute_with(|| {
// 		assert_eq!(ParaTokens::free_balance(CurrencyId::A, &sibling_d_account()),
// 500); 		assert_eq!(ParaTokens::free_balance(CurrencyId::A, &ALICE), 460);
// 	});
// }
//
